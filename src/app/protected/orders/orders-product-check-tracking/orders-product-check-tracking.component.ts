...
@Component({
  selector: 'app-orders-product-check-tracking',
  templateUrl: './orders-product-check-tracking.component.html',
  styleUrls: ['./orders-product-check-tracking.component.scss']
})
export class OrdersProductCheckTrackingComponent implements OnInit, OnDestroy {
    // add data test //
    ...
    constructor(
        ...
        this.checkTracking = false;
        this.checkStatusTracking = false;
    ...
    ngOnInit() {
        ...
        this.orderService.fetchOrder().subscribe((o: Order[]) => {
            this.orders = o;
    ...
    onScanOrdersProduct(orderODId: string) {
        ...
        // const notExistInOrders = this.orderProducts
        //     .find((p: Products) => (p.code === value && p.amount >= p.checkamount));
        const notExistInOrders = this.orders.find((order: Order) =>
                (order.orderODId === this.mID && order.orderStatusCode === 3 && order.orderStatusName  === 'pack'));


        if (notExistInOrders) {
            // console.log(notExistInOrders);
            this.checkDisable = true;
            // console.log(JSON.stringify(this.orders));
        } else {
            // console.log('false : ' + notExistInOrders);
            return;
            // console.log(JSON.stringify(this.orders));
        }

        if ((orderODId.length < 1 ) && (orderODId === '')) {
            // this.checkProducts = false;
        ...
        if (orderODId.length === 10 && notExistInOrders) {
            this.orderService.fetchViewOrderODId(this.mID)
                ...
                .subscribe((r: Order) => {
                    ...
                    if ((r[0].orderStatusCode === 3) && (r[0].orderStatusName === 'pack')) {
                        this.checkDisable = true;
                        this.checkProducts = false;
                        this.order = r;
                        ...
                        if (r[0].orderTracking !== '' && r[0].orderTracking !== undefined) {
                            // this.order.delivery.orderTracking = '';
                            this.checkStatusTracking = true;
                            this.checkTracking = false;
                            // console.log('True = ' + JSON.stringify(this.order.orderTracking));
                        } else {
                            // this.order.delivery.orderTracking = r[0].delivery[0].orderTracking;
                            this.checkStatusTracking = false;
                            this.checkTracking = true;
                            // console.log('False = ' + JSON.stringify(this.order.orderTracking));
                        }
                        // this.order.delivery.orderTracking = r[0].delivery[0].orderTracking;
                        ...
                        (<HTMLInputElement>document.getElementById('ordersODId')).value = '';
                        (<HTMLInputElement>document.getElementById('orderTracking')).focus();
    ...
    findOrderTracking(value, checkTracking) {
        // if ((this.checkDisable === true) || (value.length === 0)) {
        //     return false;
        // }
        // console.log('Value =' + value);
        // console.log('Value =' + value.length);
        // console.log(OrderEditForm.value.checkStatusTracking);
        // console.log(checkTracking);
        if ((value.length >= 4) && (value.length <= 13) && (value.length !== 0)) {

            // console.log(this.order);
            // console.log(this.order.orderStatusCode);
            // console.log(this.order.orderStatusName);
            // console.log(this.order.orderTracking);
            // console.log(this.order.delivery.orderTracking);

            const notExistInProducts = this.order.orderStatusCode === 3 && this.order.orderStatusName === 'pack';
                // && this.order.delivery.orderTracking === '' && this.order.orderTracking === '';
                // .find((p: Products) => (p.code === value && p.amount >= p.checkamount));

            // if (this.order.orderTracking === '' || this.order.orderTracking === undefined) {
            if (this.order.orderTracking === '' && this.order.delivery.orderTracking === ''
                || checkTracking || this.order.orderTracking === undefined) {
                // console.log(JSON.stringify('Yes : ' + notExistInProducts));
                this.alertSuccessSound('true');
                this.order.orderTracking = value;
                this.order.delivery.orderTracking = value;
                // console.log(this.order);
                // add historys //
                const hisName = 'ScanToTracking';
                let hisRemark = '';
                if (!this.checkStatusTracking) {
                    hisRemark += `เพิ่มสแกนการติดตามใหม่(${this.order.orderODId}) (ScanTracking Number for Pack) => "${value}"`;
                } else {
                    hisRemark += `อัพเดทสแกนการติดตาม(${this.order.orderODId}) (ScanTracking Number for Pack) => "${value}"`;
                }
                // const hisRemark = `เพิ่มสแกนการติดตาม(${this.order.orderODId})ใหม่แล้ว (ScanTracking Number for Pack) => "${value}"`;
                // this.order = order;
                this.pushHistorysDataAndUpdateTracking(this.order.orderId, hisName, hisRemark, value);
                // this.orderService.updateOrder(this.order).subscribe((r) => {
                    this.alertSuccessSound(`(${this.order.orderODId}) Update Complete => ${value}`);
                this.checkTracking = false;
                (<HTMLInputElement>document.getElementById('orderTracking')).value = '';
                (<HTMLInputElement>document.getElementById('ordersODId')).focus();
                    // console.log(this.order.orderODId + 'Update Complete => ' + '"' + value + '"');
                // });
        ...
        //     this.checkProducts = true;
        //     this.alertSuccessSound( `<<<<< COMPLETE: Update Tracking Number >>>>> \n(${this.order.orderODId}) => "${value}"`);
    ...
    // onCheckStatusTracking(checkStatusTracking) {
    //     alert(checkStatusTracking);
    // }
    ...
    public pushHistorysDataAndUpdateTracking(id, hisName, hisRemark, valueTracking) {
        // if (this.checkRoleData()) {
        // this.alertSuccess(`คุณมีสิทธื์ใช้งาน เพื่อเข้าถึงข้อมูลนี้ได้`);

        this.orderService.fetchViewOrderId(id).subscribe((r: Order) => {
            this.order = r;
            this.order.orderTracking = valueTracking;
            this.order.delivery.orderTracking = valueTracking;
            this.order.orderLastupdate = new Date();
            this.order.historys.push({
                hisName: hisName,
                hisRemark: hisRemark,
