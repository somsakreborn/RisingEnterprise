...
// ViewChild
    ...
    @ViewChild('ProductsCreateForm') ProductsCreateForm: NgForm;

    // @Input() classHeader = false;
    // @Input() cardOptionBlock = false;
    // cardToggle = 'expanded';
    count = 0;
    defualtcategorys: number;
    ...
    fetchCategory() {
        ...
        // .subscribe((r: Product[]) => this.products = r);
            .subscribe((r: Category[]) => {
                // console.log(r);
                this.categorys = r;
                this.defualtcategorys = r[0].categoryId;
                // console.log(r[0].categoryId);
                // this.ProductsCreateForm.setValue({categoryId: r[0].categoryId});
            });
    }
    ...
    onSubmit(frm) {
        // console.log(this.ProductsCreateForm);
        ...
        if (!this.product.categoryId) {
            return this.alertWarningPosition('กรุณาเลือกหมวดหมู่สินค้า', 'top');
        }

        const notExistInProductsSKU = this.products.find((p: Product) =>
            (p.productCodename.toLowerCase() === this.product.productCodename.toLowerCase()));
        if (notExistInProductsSKU) {
            // console.log(notExistInProductsSKU);
            return this.alertWarningPosition(`มีข้อมูล SKU นี้แล้วครับ => ${notExistInProductsSKU.productCodename}`, 'top');
        ...
        const hisRemark = 'สร้างสินค้าใหม่';
        // this.pushHistorysData(hisName, hisRemark);
        // const historys = {
        this.product.historys = [{
            hisName: hisName,
            hisRemark: hisRemark,
            sellerId: this.ID,
        ...
        this.fetchProduct();
        this.ProductsCreateForm.resetForm();
    }
    ...
    public checkRoleData() {
        ...
        if (roleValue === ('superadmin' || 'admin' || 'sale')) {
            // this.alertSuccess(`คุณมีสิทธื์ใช้งาน เพื่อเข้าถึงข้อมูลนี้ได้`);
            return true;
        } else {
