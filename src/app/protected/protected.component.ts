...
export class ProtectedComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // new User()
    this.user = new User();
    // this.ImageName = env.serverAPI + '/images/user/' + JSON.parse(localStorage.getItem('Image'));
    // this.ImageNameBlank = env.serverAPI + '/images/image-blank.jpg';
  ...
  ngOnInit() {
    ...
    checkImage() {
    if (this.Image === null || this.Image === '' || this.Image.length === 0) {
        // this.ImageName = 'https://somsakreborn.com:8888/images/image-blank.jpg';
        this.ImageName = env.serverAPI + '/images/no_photo.jpg';
        // this.ImageName = env.serverAPI + '/images/user/' + JSON.parse(localStorage.getItem('Image'));
  ...
  // ทำการ logout เพื่อออกจากระบบ เคลียร์ค่า SessionStorage ก่อนทำการ logout //
  onClickLogout() {
